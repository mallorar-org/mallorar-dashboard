...
class ProductVariables extends Component {
  ...
  sendVariable = () => {
    ...
    //test this
    varriableArray.forEach((x, index) => {
      var key = index;
      objectVar[key] = x;
    });
    ...
    axios
      .post("/dash/product/add/variable", variable)
      .then((res) => {
        ...
        <div className="card  my-2">
          <div className="card-body">
            <div className="d-flex col-12 align-items-center">
              <div>
                <div className="text-secondary">Variable Name</div>
                <input
              ...
              <div>
                <div className="text-secondary">Variants</div>
                <input
                  onChange={this.onCh}
                  className="form-control"
                  name="variableValue"
              ...
              <div>
                ...
                <button
                  ...
                  className="button button-primary ml-2 btn btn-primary "
                >
                  Add
                </button>
              </div>
              <div>
                <div>.</div>
                <button
                  onClick={this.cancelC}
                  className=" button ml-2 btn btn-danger "
                >
                  Cancel
                </button>
              </div>
  ...
  variables = () => {
    if (this.state.variables.length > 0) {
      return this.state.variables.map((vr, index) => (
